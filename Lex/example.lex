...
void check__int(char *s,int lineCount,int charCount)
{
	printf("\n\ncheck__int\n\n");
	printf("\n int is : %s\n\n",s);
	int str_l=strlen(s);
	...
	*/
	for(i=0;i<str_l;i++)
		if( isdigit(s[i-1]) && s[i] == 43 && isdigit(s[i+1]) )
			{
			printf(" %c is integer.\n",s[i-1]);
			printf("Line:%d,1st char:%d,\"%d\" is an \"Integer\".\n",lineCount,charCount++,s[i-1]);
			printf("Line:%d,1st char:%d,\"%c\" is  \"symbol\".\n",lineCount,charCount++,s[i-1]);
			printf("Line:%d,1st char:%d,\"%d\" is an \"Integer\".\n",lineCount,charCount++,s[i-1]);
			}
		else
		{
...
printf("\ncharCount : %d",charCount);
printf("\n-----------\n");
check__int(yytext,lineCount,charCount);
charCount += yyleng;
}
